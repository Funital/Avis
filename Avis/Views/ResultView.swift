import SwiftUI

struct ResultView: View {
    let result: QuizResult
    let onRetry: () -> Void
    let onRetryWrong: () -> Void
    let onDismiss: () -> Void
    
    var body: some View {
        NavigationStack {
            ScrollView {
                VStack(spacing: 24) {
                    // 점수 원형
                    scoreCircle
                    
                    // 등급 메시지
                    Text(result.grade)
                        .font(.title2)
                        .fontWeight(.bold)
                    
                    // 상세 통계
                    statsCards
                    
                    // 틀린 단어 목록
                    if !result.wrongWords.isEmpty {
                        wrongWordsList
                    }
                    
                    // 버튼들
                    actionButtons
                }
                .padding()
            }
            .background(Color(.systemGroupedBackground))
            .navigationTitle("퀴즈 결과")
            .navigationBarTitleDisplayMode(.inline)
        }
    }
    
    private var scoreCircle: some View {
        CircularProgressView(
            progress: result.score / 100,
            size: 160,
            lineWidth: 16,
            colors: scoreColors,
            label: "\(Int(result.score))%",
            sublabel: "정확도"
        )
        .padding(.top)
    }
    
    private var scoreColors: [Color] {
        switch result.score {
        case 90...100: return [.green, .mint]
        case 70..<90: return [.indigo, .purple]
        case 50..<70: return [.orange, .yellow]
        default: return [.red, .orange]
        }
    }
    
    private var statsCards: some View {
        HStack(spacing: 12) {
            StatBadge(icon: "doc.text", value: "\(result.totalQuestions)", label: "총 문제", color: .indigo)
            StatBadge(icon: "checkmark.circle", value: "\(result.correctAnswers)", label: "정답", color: .green)
            StatBadge(icon: "xmark.circle", value: "\(result.totalQuestions - result.correctAnswers)", label: "오답", color: .red)
        }
    }
    
    private var wrongWordsList: some View {
        VStack(alignment: .leading, spacing: 12) {
            Text("틀린 단어 (\(result.wrongWords.count)개)")
                .font(.headline)
                .padding(.leading, 4)
            
            VStack(spacing: 0) {
                ForEach(Array(result.wrongWords.enumerated()), id: \.offset) { index, word in
                    HStack {
                        Text(word.english)
                            .font(.subheadline)
                            .fontWeight(.medium)
                        Spacer()
                        Text(word.korean)
                            .font(.subheadline)
                            .foregroundColor(.secondary)
                    }
                    .padding(.horizontal, 16)
                    .padding(.vertical, 12)
                    
                    if index < result.wrongWords.count - 1 {
                        Divider().padding(.leading, 16)
                    }
                }
            }
            .cardStyle(hasShadow: false)
        }
    }
    
    private var actionButtons: some View {
        VStack(spacing: 12) {
            Button {
                onRetry()
            } label: {
                Label("다시 풀기", systemImage: "arrow.counterclockwise")
            }
            .buttonStyle(PrimaryButtonStyle())
            
            if !result.wrongWords.isEmpty {
                Button {
                    onRetryWrong()
                } label: {
                    Label("틀린 단어만 다시", systemImage: "xmark.circle")
                }
                .buttonStyle(DestructiveButtonStyle())
            }
            
            Button {
                onDismiss()
            } label: {
                Text("홈으로")
            }
            .buttonStyle(SecondaryButtonStyle())
        }
    }
}
